...
class ParrotAnafiController(BaseDrone):
    ...
    def __init__(self, ip_address: str = "10.202.0.1", simulation_mode: bool = True):
        """
        Inicializa el controlador de drones Parrot ANAFI.
        
        Args:
            ip_address: Dirección IP del drone (default es la IP estándar de ANAFI)
            simulation_mode: Si True, funciona en modo simulación incluso si Olympe está disponible
        """
    ...
    def connect(self) -> bool:
        ...
        try:
            if not OLYMPE_AVAILABLE or self.simulation_mode:
                # Simulación si Olympe no está disponible o si estamos en modo simulación
                self.connected = True
